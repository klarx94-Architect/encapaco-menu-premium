...
import { checkPassword, fetchMenu, saveMenu, uploadCoverImage, subscribeMenu } from '../lib/menuService';

const ADMIN_STORAGE_KEY = 'encapaco_admin_auth';

const EMPTY_ITEM = {
  id: '', name_es: '', name_en: '', name_fr: '',
  description_es: '', price: '', visible: true
};
...
export default function AdminPaco() {
  ...
  const updateMenuData = (newMenu) => setMenuData(JSON.parse(JSON.stringify(newMenu)));
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      alert('Error al guardar en Firebase. Revisa la conexión.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const setActiveCover = (catId, imageUrl) => {
    ...
    if (cat) cat.cover_image = imageUrl;
  ...
  const handleCoverImageUpload = async (e, catId) => {
    ...
    try {
      const url = await uploadCoverImage(catId, file);
      const newData = { ...menuData };
      const cat = newData.categories.find(c => c.id === catId);
      if (cat) {
        cat.cover_images = [...(cat.cover_images || []), url];
        cat.cover_image = url;
      }
      updateMenuData(newData);
    } catch (err) {
      alert('Error subiendo imagen a Firebase Storage.');
    } finally {
  ...
  const handleDeleteCoverImage = (catId, imgIdx) => {
    ...
    const cat = newData.categories.find(c => c.id === catId);
    if (!cat) return;
    const deleted = cat.cover_images[imgIdx];
    cat.cover_images = cat.cover_images.filter((_, i) => i !== imgIdx);
    if (cat.cover_image === deleted) cat.cover_image = cat.cover_images[0] || '';
    updateMenuData(newData);
  };
  ...
  const toggleCategoryVisibility = (catId) => {
    const newData = { ...menuData };
    const cat = newData.categories.find(c => c.id === catId);
    if (cat) cat.visible = cat.visible === false ? true : false;
    updateMenuData(newData);
  };
  ...
  const handleUpdateItem = (catId) => {
    ...
    if (idx !== -1) cat.items[idx] = { ...editingItem };
    updateMenuData(newData);
    setEditingItem(null);
  };
  ...
  const handleDeleteItem = (catId, itemId) => {
    if (!window.confirm('¿Eliminar este plato? Se guardará al pulsar "Guardar todo".')) return;
    const newData = { ...menuData };
    const cat = newData.categories.find(c => c.id === catId);
    ...
    return (
      <div className="min-h-screen bg-pearl-white flex flex-col items-center justify-center p-4 sm:p-6 overflow-hidden relative">
        ...
        >
          ...
          <p className="mt-12 text-center text-[10px] uppercase tracking-widest font-black text-neutral-dark/20">Protocolo Senior Gastro v3.0 · Firebase</p>
        </motion.div>
      </div>
    );
  }
      ...
      <nav className="fixed bottom-0 left-0 w-full h-20 lg:h-full lg:w-72 bg-white border-t lg:border-t-0 lg:border-r border-black/5 p-4 lg:p-6 z-[600] flex flex-row lg:flex-col justify-between items-center lg:items-stretch">
        <div className="flex flex-row lg:flex-col items-center lg:items-stretch gap-6 lg:gap-12 w-full lg:w-auto">
          <div className="flex items-center gap-4 px-2">
            <div className="w-10 h-10 bg-sierra-gold rounded-xl flex items-center justify-center shadow-lg">
      ...
      <AnimatePresence>
        {showSuccess && (
          ...
          >
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-1">Firebase · Tiempo Real</p>
              <p className="font-serif font-black text-sm">Carta guardada en Firestore</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <main className="lg:pl-72 p-6 md:p-12 pb-32 mb-20 lg:mb-0">
        <header className="mb-16 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div>
            ...
            <p className="text-neutral-dark/40 italic font-serif text-lg">Gestiona portadas de categorías y platos · Firebase</p>
          </div>
          <button
        ...
        {loading ? (
          <div className="h-64 flex flex-col items-center justify-center gap-6 text-neutral-dark/20 uppercase tracking-[0.4em] text-xs font-black">
            <Loader2 className="animate-spin" size={32} /><span>Conectando con Firebase...</span>
          </div>
        ) : !menuData ? (
          <div className="h-64 flex flex-col items-center justify-center gap-4 text-neutral-dark/30 text-center">
            <AlertCircle size={40} />
            <p className="font-serif font-black text-lg">Sin datos en Firestore</p>
            <p className="text-sm">Sube el menu.json inicial desde la consola de Firebase.</p>
          </div>
        ) : (
              ...
              <motion.div key={category.id} className="bg-white rounded-[2.5rem] border border-black/5 shadow-sm overflow-hidden flex flex-col">
                ...
                <div className="px-6 pt-4 pb-2">
                  <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                      ...
                      <div key={idx} className="relative shrink-0">
                        <button
                          ...
                          className={`w-14 h-14 rounded-xl overflow-hidden border-2 transition-all ${
                            category.cover_image === img ? 'border-sierra-gold' : 'border-transparent opacity-60 hover:opacity-100'
                          }`}
                        >
                    ...
                    <label className="shrink-0 w-14 h-14 rounded-xl border-2 border-dashed border-black/10 flex items-center justify-center cursor-pointer hover:border-sierra-gold/40 hover:bg-sierra-gold/5 transition-all">
                      {uploadingCover === category.id
                        ? <Loader2 size={16} className="animate-spin text-sierra-gold" />
                ...
                <div className="px-6 pb-6 mt-auto flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-sierra-gold hover:text-neutral-dark transition-colors"
                  >
          ...
          return (
            <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center p-0 sm:p-6">
              <motion.div
                ...
                onClick={() => { setExpandedCategory(null); setEditingItem(null); setShowAddItem(false); }}
              ...
              >
                <div className="p-8 border-b border-black/5 flex items-center justify-between shrink-0">
                  <div>
                    <p className="text-[9px] font-black uppercase tracking-[0.3em] text-sierra-gold mb-1">Categoría</p>
                    <h2 className="text-2xl font-serif font-black">{cat.name_es}</h2>
                  </div>
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => { setShowAddItem(true); setEditingItem({ ...EMPTY_ITEM }); }}
                      className="flex items-center gap-2 px-5 py-3 bg-sierra-gold text-neutral-dark rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg hover:scale-105 transition-all"
                    >
                      <Plus size={14} />Añadir plato
                    </button>
                    <button
                      onClick={() => { setExpandedCategory(null); setEditingItem(null); setShowAddItem(false); }}
                      className="text-neutral-dark/20 hover:text-neutral-dark p-2 transition-colors"
                    >
                      <X size={24} />
                    </button>
                  </div>
                ...
                <div className="overflow-y-auto flex-1 p-6 space-y-3">
                  {showAddItem && (
                    <motion.div
                      ...
                      className="bg-sierra-gold/5 border border-sierra-gold/20 rounded-3xl p-6 space-y-4 overflow-hidden"
                    >
                      <p className="text-[10px] font-black uppercase tracking-widest text-sierra-gold">Nuevo plato</p>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="col-span-2">
                          <label className="text-[9px] font-black uppercase tracking-widest text-neutral-dark/40 mb-2 block">Nombre *</label>
                          <input type="text" placeholder="Ej: Pollo completo" value={editingItem?.name_es || ''}
                            onChange={(e) => setEditingItem(prev => ({ ...prev, name_es: e.target.value }))}
                            className="w-full px-5 py-3 rounded-xl bg-white border border-black/5 focus:outline-none font-bold text-sm" />
                        </div>
                        <div>
                          <label className="text-[9px] font-black uppercase tracking-widest text-neutral-dark/40 mb-2 block">Precio</label>
                          <input type="text" placeholder="Ej: 5,50€" value={editingItem?.price || ''}
                            onChange={(e) => setEditingItem(prev => ({ ...prev, price: e.target.value }))}
                            className="w-full px-5 py-3 rounded-xl bg-white border border-black/5 focus:outline-none font-black text-sierra-gold text-sm" />
                        </div>
                        <div className="col-span-2">
                          <label className="text-[9px] font-black uppercase tracking-widest text-neutral-dark/40 mb-2 block">Descripción</label>
                          <input type="text" placeholder="Ej: Con tomate y lechuga" value={editingItem?.description_es || ''}
                            onChange={(e) => setEditingItem(prev => ({ ...prev, description_es: e.target.value }))}
                            className="w-full px-5 py-3 rounded-xl bg-white border border-black/5 focus:outline-none font-serif italic text-sm" />
                        </div>
                      </div>
                      <div className="flex gap-3 pt-2">
                        ...
                        <button onClick={() => { setShowAddItem(false); setEditingItem(null); }} className="px-6 py-3 rounded-full border border-black/10 text-neutral-dark/40 text-[10px] font-black uppercase tracking-widest">Cancelar</button>
                      </div>
                    </motion.div>
                  )}
                  ...
                  {cat.items?.map((item) => (
                    <div
                      ...
                      className={`flex items-center gap-4 p-4 rounded-2xl border transition-all ${
                        item.visible === false
                          ? 'border-dashed border-black/5 bg-black/[0.02] opacity-60'
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        {editingItem?.id === item.id && editingItem?.catId === cat.id ? (
                          <div className="space-y-2">
                            <input type="text" value={editingItem.name_es}
                              onChange={(e) => setEditingItem(prev => ({ ...prev, name_es: e.target.value }))}
                              className="w-full px-3 py-2 rounded-xl bg-pearl-white border border-black/5 font-bold text-sm" />
                            <div className="flex gap-2">
                              <input type="text" value={editingItem.price}
                                onChange={(e) => setEditingItem(prev => ({ ...prev, price: e.target.value }))}
                                className="w-28 px-3 py-2 rounded-xl bg-pearl-white border border-black/5 font-black text-sierra-gold text-sm" />
                              <input type="text" value={editingItem.description_es || ''}
                                onChange={(e) => setEditingItem(prev => ({ ...prev, description_es: e.target.value }))}
                                className="flex-1 px-3 py-2 rounded-xl bg-pearl-white border border-black/5 font-serif italic text-sm" />
                            </div>
                      ...
                      {editingItem?.id !== item.id && (
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <button onClick={() => toggleItemVisibility(cat.id, item.id)} className={`p-2 rounded-full transition-colors ${item.visible === false ? 'text-neutral-dark/20 hover:bg-black/5' : 'text-green-500 hover:bg-green-500/10'}`}>
